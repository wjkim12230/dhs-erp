...
export default function SpecificationDetailListPage() {
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'orderingValue', headerName: '수주값', flex: 1 },
  ...
  return (
    <Box>
      <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', mb:2 }}>
        <Typography variant="h5" fontWeight={700}>사양 상세</Typography>
        <Button variant="contained" startIcon={<Add />} onClick={() => { setForm({name:'',orderingValue:'',priority:0}); setOpen(true); }}>추가</Button>
      </Box>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent sx={{ pt:'16px !important' }}>
          ...
          <TextField label="수주값" value={form.orderingValue} onChange={(e) => setForm(p=>({...p,orderingValue:e.target.value}))} fullWidth sx={{mb:2}} />
